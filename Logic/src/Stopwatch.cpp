...
float logic::Stopwatch::get_delta_time() {
...
float logic::Stopwatch::get_start_time() {
    std::chrono::system_clock::time_point now = std::chrono::high_resolution_clock::now();
    std::chrono::duration<float, std::milli> delta = now - t_start; // difference in milliseconds
    return delta.count();
}

float logic::Stopwatch::get_time_between(std::chrono::system_clock::time_point time_point1,
