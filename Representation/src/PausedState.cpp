...
std::pair<int, std::unique_ptr<State>> PausedState::proces_user_input(const sf::Event& event, sf::RenderWindow& window) {
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
        ...
        Coordinate lower_right_corner1(bounds_resume.left + bounds_resume.width, bounds_resume.top + bounds_resume.height);
...
void PausedState::render(sf::RenderWindow& window) {
    ...
    text.setPosition({window_width/2-12, window_height-2*float(character_size)-character_size}); // set position of the text to center of screen
    ...
    text2.setPosition({window_width2/2-12, window_height2-float(character_size2)}); // set position of the text to center of screen
