...
void representation::PausedState::updateLayout(sf::Vector2u windowSize) {
    ...
    menuButtonBackground_.setOrigin(menuButtonBackground_.getSize() / 2.f +
                                    menuButtonBackground_.getGlobalBounds().getPosition());
    ...
    resumeButtonText_.setOrigin(bannerTextBounds.left + bannerTextBounds.width / 2.0f,
                                bannerTextBounds.top + bannerTextBounds.height / 2.0f);
    sf::Vector2f pos2 = {float(windowSize.x / 2 - 12),
                         windowSize.y - 2 * float(character_size2) - character_size2 * float(1.5)};
    resumeButtonText_.setPosition(pos2);

    resumeButtonBackground_.setSize({character_size2 * 6.f, character_size2 * 2.f});
    resumeButtonBackground_.setOrigin(resumeButtonBackground_.getSize() / 2.f +
                                      resumeButtonBackground_.getGlobalBounds().getPosition());
